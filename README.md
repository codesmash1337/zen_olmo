...
#### Usage
```bash
# 1. Process URLs from default file (data/high_quality_texts.txt) and append to output
python scripts/fetch_parse_texts.py

# 2. Overwrite the output file instead of appending
python scripts/fetch_parse_texts.py --overwrite

# 3. Use a different input file
python scripts/fetch_parse_texts.py --file data/texts.txt

# 4. Specify a different output file
python scripts/fetch_parse_texts.py --output data/yoda.txt
```

### ```data/high_quality_texts.txt```
